...
export default function Login() {
  ...
  async function recordSignIn() {
    const { error: auditError } = await supabase.rpc('record_platform_sign_in', { p_actor_type: 'admin' })
    if (auditError) console.warn('Could not record admin sign-in', auditError)
  }
  ...
  return (
    <main className="admin-auth-page">
      <section className="admin-auth-intro">
        ...
        <div><span className="admin-kicker">Platform operations</span><h1>The control room for ordered.food.</h1><p>Review restaurants, monitor live operations and keep every administrative decision accountable.</p></div>
        <small>Restricted access · Authorised administrators only</small>
      </section>
      <section className="admin-auth-panel"><div className="admin-auth-card">
        <span className="admin-kicker">Secure admin access</span><h2>{mode === 'sign-in' ? 'Welcome back' : 'Activate your account'}</h2><p>{mode === 'sign-in' ? 'Sign in with your platform administrator account.' : 'This only works for an email that has already been invited.'}</p>
        <div className="auth-mode-switch" role="tablist"><button type="button" className={mode === 'sign-in' ? 'active' : ''} onClick={() => { setMode('sign-in'); setError(''); setMessage('') }}>Sign in</button><button type="button" className={mode === 'activate' ? 'active' : ''} onClick={() => { setMode('activate'); setError(''); setMessage('') }}>First access</button></div>
