...
export default function Checkout() {
  ...
  useEffect(() => {
    ...
    if (savedCheckout) {
      try {
        ...
        if (parsed.method === 'delivery' || parsed.method === 'collection') setMethod(parsed.method)
      } catch { window.localStorage.removeItem(`ordered-food-checkout:${slug}`) }
    }
    const pendingOrder = window.localStorage.getItem(`ordered-food-pending-order:${slug}`)
    if (pendingOrder) {
      try {
        const parsed = JSON.parse(pendingOrder) as CreatedOrder
        if (parsed.order_id && parsed.payment_status !== 'paid') setCreatedOrder(parsed)
        else window.localStorage.removeItem(`ordered-food-pending-order:${slug}`)
      } catch { window.localStorage.removeItem(`ordered-food-pending-order:${slug}`) }
    }
    ...
    async function loadPage() {
      ...
      setError('')
      try {
        const [{ data, error: rpcError }, { data: sessionData, error: sessionError }] = await Promise.all([
        ...
        if (user) {
          ...
          await Promise.allSettled([supabase.rpc('claim_customer_orders'), loadCustomerAddresses(user.id)])
        }
      } catch (loadError) {
        setError(loadError instanceof Error ? loadError.message : 'Unable to load checkout. Please refresh and try again.')
      } finally {
        setLoading(false)
      }
    }
  ...
  async function authenticateCustomer() {
    ...
    if (accountMode === 'signin') {
      const { data, error: signInError } = await supabase.auth.signInWithPassword({ email: details.email.trim().toLowerCase(), password })
      if (signInError) {
    ...
    if (!data.session) {
      setError('Check your email to confirm your account, then return here and choose Already registered to continue.')
      return false
    }
    setSignedIn(true)
    await supabase.rpc('claim_customer_orders')
  ...
  async function saveProfile() {
    ...
    await supabase.from('customer_profiles').upsert({
      ...
      postcode: method === 'delivery' ? formatPostcode(details.postcode) : null,
    ...
    if (method === 'delivery' && selectedAddressId === 'new') {
      const address = {
        ...
        postcode: formatPostcode(details.postcode),
  ...
  async function openSecurePayment(order: CreatedOrder) {
    setSubmitting(true)
    setError('')
    setMessage(`Opening secure payment for order #${order.order_number}…`)
    ...
    if (checkoutError || !checkoutData?.checkout_url) {
      setSubmitting(false)
      setMessage('')
      setError(checkoutData?.error || checkoutError?.message || 'Unable to open secure payment. Your order has been saved, so you can safely retry without creating another order.')
      return
    }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (createdOrder) {
      await openSecurePayment(createdOrder)
      return
    }
    ...
    if (!details.firstName.trim() || !details.lastName.trim() || !details.email.trim() || !details.phone.trim() || !requiredDeliveryFields) {
    ...
    const { data, error: orderError } = await supabase.rpc('create_order', {
      ...
      customer_first_name: details.firstName.trim(), customer_last_name: details.lastName.trim(),
      customer_email: details.email.trim().toLowerCase(), customer_phone: details.phone.trim(),
      basket_items: basketLines.map((line) => ({
        ...
        special_instructions: line.special_instructions?.trim() || null,
      })),
      address_line_1: method === 'delivery' ? details.addressLine1.trim() : null,
      address_line_2: method === 'delivery' ? details.addressLine2.trim() || null : null,
      town_city: method === 'delivery' ? details.town.trim() : null,
      postcode: method === 'delivery' ? formatPostcode(details.postcode) : null,
      delivery_instructions: method === 'delivery' ? details.instructions.trim() || null : null,
    })
    ...
    <main className="checkout-page">
      ...
      <div className="checkout-layout">
        <form className="checkout-form" onSubmit={handleSubmit} noValidate>
          ...
          <section className="checkout-card"><span className="checkout-step">2</span><div className="checkout-section-heading"><h2>Your details</h2><p>We will use these details for order updates.</p></div><div className="checkout-fields two-column"><label>First name<input value={details.firstName} onChange={(event) => updateField('firstName', event.target.value)} autoComplete="given-name" required disabled={submitting || Boolean(createdOrder)} /></label><label>Last name<input value={details.lastName} onChange={(event) => updateField('lastName', event.target.value)} autoComplete="family-name" required disabled={submitting || Boolean(createdOrder)} /></label><label>Email<input type="email" value={details.email} onChange={(event) => updateField('email', event.target.value)} autoComplete="email" required disabled={signedIn || submitting || Boolean(createdOrder)} /></label><label>Mobile number<input type="tel" value={details.phone} onChange={(event) => updateField('phone', event.target.value)} autoComplete="tel" required disabled={submitting || Boolean(createdOrder)} /></label></div></section>
          {method === 'delivery' && <section className="checkout-card"><span className="checkout-step">{addressStep}</span><div className="checkout-section-heading"><h2>Delivery address</h2><p>{signedIn && savedAddresses.length ? 'Choose a saved address or enter a different one.' : 'We will save this address to your account for next time.'}</p></div>{signedIn && savedAddresses.length > 0 && <div className="checkout-saved-addresses">{savedAddresses.map((address) => <button className={selectedAddressId === address.id ? 'selected' : ''} type="button" key={address.id} onClick={() => applyAddress(address)} disabled={submitting || Boolean(createdOrder)}><span className="checkout-address-radio" aria-hidden="true" /><span><strong>{address.label}{address.is_default ? ' · Default' : ''}</strong><small>{address.address_line_1}{address.address_line_2 ? `, ${address.address_line_2}` : ''}</small><small>{address.town_city}, {address.postcode}</small>{address.delivery_instructions && <small>Note: {address.delivery_instructions}</small>}</span></button>)}<button className={selectedAddressId === 'new' ? 'selected' : ''} type="button" onClick={chooseNewAddress} disabled={submitting || Boolean(createdOrder)}><span className="checkout-address-radio" aria-hidden="true" /><span><strong>Use a different address</strong><small>Enter another delivery address below</small></span></button><Link className="checkout-manage-addresses" to="/account/addresses">Manage saved addresses</Link></div>}<div className="checkout-fields"><label>Address line 1<input value={details.addressLine1} onChange={(event) => updateField('addressLine1', event.target.value)} autoComplete="address-line1" required disabled={submitting || Boolean(createdOrder)} /></label><label>Address line 2 <span>Optional</span><input value={details.addressLine2} onChange={(event) => updateField('addressLine2', event.target.value)} autoComplete="address-line2" disabled={submitting || Boolean(createdOrder)} /></label><div className="two-column"><label>Town or city<input value={details.town} onChange={(event) => updateField('town', event.target.value)} autoComplete="address-level2" required disabled={submitting || Boolean(createdOrder)} /></label><label>Postcode<input value={details.postcode} onChange={(event) => updateField('postcode', formatPostcode(event.target.value))} autoComplete="postal-code" required disabled={submitting || Boolean(createdOrder)} /></label></div><label>Delivery instructions <span>Optional</span><textarea value={details.instructions} onChange={(event) => updateField('instructions', event.target.value)} rows={3} placeholder="Door number, access instructions or anything the driver should know" disabled={submitting || Boolean(createdOrder)} /></label></div></section>}
          <section className="checkout-card"><span className="checkout-step">{accountStep}</span><div className="checkout-section-heading"><h2>{signedIn ? 'Your account' : 'Save your details'}</h2><p>{signedIn ? 'You are signed in. This order will appear in My orders.' : 'Create an account to save your details and see your full order history.'}</p></div>{signedIn ? <div className="payment-placeholder">Signed in as {details.email}</div> : <div className="checkout-account"><div className="checkout-account-tabs"><button type="button" className={accountMode === 'create' ? 'selected' : ''} onClick={() => setAccountMode('create')} disabled={submitting || Boolean(createdOrder)}>Create account</button><button type="button" className={accountMode === 'signin' ? 'selected' : ''} onClick={() => setAccountMode('signin')} disabled={submitting || Boolean(createdOrder)}>Already registered</button></div><div className="checkout-fields"><label>Password<input type="password" minLength={8} autoComplete={accountMode === 'create' ? 'new-password' : 'current-password'} value={password} onChange={(event) => setPassword(event.target.value)} required disabled={submitting || Boolean(createdOrder)} /><span>At least 8 characters</span></label></div></div>}</section>
          <section className="checkout-card checkout-payment-placeholder"><span className="checkout-step">{paymentStep}</span><div className="checkout-section-heading"><h2>Payment</h2><p>You will be transferred to Stripe for secure card, Apple Pay or Google Pay payment.</p></div><div className="payment-placeholder">{createdOrder ? `Order #${createdOrder.order_number} is ready for secure payment` : 'Payment details are entered securely on Stripe'}</div></section>
          {submitted && minimumShortfall > 0 && <p className="checkout-error">Add {money.format(minimumShortfall / 100)} more to meet the minimum order.</p>}{error && <p className="checkout-error">{error}</p>}{message && <p className="checkout-message">{message}</p>}<button className="checkout-submit" type="submit" disabled={minimumShortfall > 0 || submitting}>{submitting ? 'Opening secure payment…' : createdOrder ? `Retry payment · ${money.format(createdOrder.total_pence / 100)}` : `Pay securely · ${money.format(total / 100)}`}</button>
        </form>
