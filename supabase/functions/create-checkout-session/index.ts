...
function respond(request: Request, body: unknown, status = 200, extraHeaders: Record<string, string> = {}) {
  return new Response(JSON.stringify(body), {
    status,
    headers: { ...cors(request), ...extraHeaders, 'Content-Type': 'application/json', 'Cache-Control': 'no-store' },
  })
}
...
function errorDetails(error: unknown) {
  ...
  if (error && typeof error === 'object') {
    ...
    return {
      message: typeof value.message === 'string' ? value.message : typeof raw?.message === 'string' ? raw.message : 'Payment provider returned an unknown error.',
...
Deno.serve(async (request) => {
  ...
  if (Number.isFinite(contentLength) && contentLength > MAX_BODY_BYTES) return respond(request, { error: 'Request body is too large.' }, 413)
  ...
  if (new TextEncoder().encode(rawBody).byteLength > MAX_BODY_BYTES) return respond(request, { error: 'Request body is too large.' }, 413)
  ...
  if (!UUID_PATTERN.test(orderId)) return respond(request, { error: 'A valid order_id is required.' }, 400)

  const db = createClient(supabaseUrl, serviceRoleKey, { auth: { persistSession: false, autoRefreshToken: false } })
  const stripe = new Stripe(stripeKey)

  try {
    const limit = await consumeLimit(db, `order:${orderId}`, 10 * 60, 10)
    if (!limit.allowed) {
      return respond(request, { error: 'Too many payment attempts for this order. Please wait before trying again.' }, 429, {
        'Retry-After': String(Math.max(1, limit.retry_after_seconds)),
      })
    }
  } catch (error) {
    console.error('Checkout rate limit check failed', error)
    return respond(request, { error: 'Payment controls could not be verified. Please try again.' }, 503)
  }
  ...
  try {
    ...
    if (order.stripe_checkout_session_id) {
      try {
        ...
        if (existing.status === 'open' && existing.url) return respond(request, { checkout_url: existing.url, session_id: existing.id, payout_mode: payoutMode })
      } catch {}
    }
    ...
    const storedNet = Number(order.restaurant_net_pence ?? 0)
    const restaurantNet = Math.min(Number(order.total_pence), storedNet > 0 ? storedNet : calculatedNet)
    const applicationFee = Math.max(0, Number(order.total_pence) - restaurantNet)
    ...
    const metadata = {
      ...
      discount_pence: String(order.discount_pence ?? 0),
    }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${siteUrl}/order/success?session_id={CHECKOUT_SESSION_ID}&order_id=${order.id}`,
      cancel_url: `${siteUrl}/r/${restaurant.slug}/checkout?payment=cancelled`,
      client_reference_id: order.id,
      metadata,
      payment_intent_data: paymentIntentData,
      expires_at: Math.floor(Date.now() / 1000) + 1800,
    }, { idempotencyKey: `ordered-food-authorise-v5-${payoutMode}-${order.id}-${order.total_pence}` })
    ...
    console.error('Checkout failure', JSON.stringify(details))
    return respond(request, { error: 'Unable to start secure payment. Please try again.', details: { type: details.type, code: details.code } }, 500)
  }
})
