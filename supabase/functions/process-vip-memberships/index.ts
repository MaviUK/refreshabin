...
Deno.serve(async(req)=>{
  ...
  if(!url||!service||!resend)return new Response(JSON.stringify({error:'VIP processor is not configured'}),{status:500,headers:{'Content-Type':'application/json'}})
  const db=createClient(url,service,{auth:{persistSession:false,autoRefreshToken:false}})
  const{data:membership,error:membershipError}=await db.rpc('process_vip_memberships',{p_limit:1000})
  if(membershipError)return new Response(JSON.stringify({error:membershipError.message}),{status:500,headers:{'Content-Type':'application/json'}})
  const{data:rows,error}=await db.from('reward_notification_queue').select('id,reward_issuance_id,vip_tier_history_id,customer_user_id,event_type,subject,body,action_url,push_payload,push_status,attempts').like('event_type','vip_%').in('status',['pending','failed']).lte('available_at',new Date().toISOString()).lt('attempts',5).order('created_at',{ascending:true}).limit(100)
  ...
  for(const row of rows||[]){
    try{
      ...
      const action=`${site}${row.action_url||'/account/vip'}`
      const html=`<div style="font-family:Arial,sans-serif;background:#f5f1e8;padding:32px"><div style="max-width:560px;margin:auto;background:#fff;border-radius:20px;padding:32px"><p style="font-size:13px;text-transform:uppercase;letter-spacing:.12em;color:#756d63">ordered.food VIP</p><h1 style="font-size:30px">${esc(row.subject)}</h1><p style="font-size:17px;line-height:1.6;color:#4b453e">${esc(row.body)}</p><a href="${esc(action)}" style="display:inline-block;margin-top:18px;background:#171615;color:#fff;padding:13px 20px;border-radius:999px;text-decoration:none">View VIP membership</a></div></div>`
      await sendEmail(resend,from,userData.user.email,row.subject,html)
      let pushStatus='skipped'
      if(pushUrl){try{await dispatchPush(pushUrl,pushSecret,{customer_user_id:row.customer_user_id,event_type:row.event_type,...(row.push_payload||{})});pushStatus='dispatched'}catch(pushError){pushStatus='failed';console.error('VIP push hook failed',pushError)}}
    ...
    }catch(caught){const message=caught instanceof Error?caught.message.slice(0,500):'VIP notification failed';const attempts=(row.attempts||0)+1;await db.from('reward_notification_queue').update({status:attempts>=5?'failed':'pending',last_error:message,available_at:new Date(Date.now()+60*60*1000).toISOString(),updated_at:new Date().toISOString()}).eq('id',row.id).eq('status','processing');results.push({id:row.id,status:'failed',error:message})}
  }
  return new Response(JSON.stringify({membership,processed_notifications:results.length,results}),{headers:{'Content-Type':'application/json','Cache-Control':'no-store'}})
})
