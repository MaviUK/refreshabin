...
begin
  ...
  select jsonb_build_object(
    'summary', jsonb_build_object(
      ...
      'discount_given_pence', coalesce((select sum(pr.discount_pence) from public.promotion_redemptions pr where pr.restaurant_id = restaurant_id), 0)
    ),
    'promotions', coalesce(jsonb_agg(jsonb_build_object(
      ...
      'percentage_basis_points', p.percentage_basis_points,
      'fixed_discount_pence', p.fixed_discount_pence,
      'minimum_order_pence', p.minimum_order_pence,
      'maximum_discount_pence', p.maximum_discount_pence,
      'starts_at', p.starts_at,
      'ends_at', p.ends_at,
      'total_redemption_limit', p.total_redemption_limit,
      'per_customer_limit', p.per_customer_limit,
      'redemption_count', p.redemption_count,
      'first_order_only', p.first_order_only,
      'fulfilment_methods', p.fulfilment_methods,
...
create or replace function public.create_restaurant_promotion(
  ...
  p_fixed_discount_pence integer default null,
  p_minimum_order_pence integer default 0,
  p_maximum_discount_pence integer default null,
...
begin
  ...
  if not public.restaurant_has_subscription_feature('marketing') then
    raise exception 'Marketing tools require a Growth or Pro subscription' using errcode = '42501';
  end if;
  ...
  insert into public.restaurant_promotions(
    restaurant_id, name, code, promotion_type, percentage_basis_points,
    fixed_discount_pence, minimum_order_pence, maximum_discount_pence,
    starts_at, ends_at, total_redemption_limit, per_customer_limit,
    first_order_only, fulfilment_methods, created_by
  ) values (
    restaurant_id, trim(p_name), upper(trim(p_code)), p_promotion_type,
    p_percentage_basis_points, p_fixed_discount_pence,
    greatest(coalesce(p_minimum_order_pence, 0), 0), p_maximum_discount_pence,
    coalesce(p_starts_at, now()), p_ends_at, p_total_redemption_limit,
...
begin
  ...
  where id = p_promotion_id and restaurant_id = restaurant_id;
