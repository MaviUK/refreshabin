...
begin
  ...
  if p.campaign_referral_cap is not null and total_count>=p.campaign_referral_cap then
    ...
    raise exception 'This referral campaign has reached its limit';
  end if;
  if p.maximum_referrals_per_customer is not null and (select count(*) from public.customer_referrals x where x.program_id=p.id and x.referrer_user_id=c.customer_user_id and x.status<>'rejected')>=p.maximum_referrals_per_customer then raise exception 'This referrer has reached the programme limit'; end if;
