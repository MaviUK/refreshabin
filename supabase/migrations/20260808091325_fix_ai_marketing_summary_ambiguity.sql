...
begin
  ...
  report_summary:=coalesce(base->'summary','{}'::jsonb);
  sent:=coalesce((report_summary->>'sent')::numeric,0);
  opened:=coalesce((report_summary->>'opened')::numeric,0);
  orders_generated:=coalesce((report_summary->>'orders_generated')::numeric,0);
  revenue:=coalesce((report_summary->>'revenue_generated_pence')::numeric,0);
  reward_cost:=coalesce((report_summary->>'reward_cost_pence')::numeric,0);
  ...
  return base || jsonb_build_object(
    'ai_summary',jsonb_build_object(
      ...
      'retention_percent',coalesce((report_summary->>'retention_percent')::numeric,0),
      'reactivation_percent',coalesce((report_summary->>'reactivation_percent')::numeric,0)
    ),
